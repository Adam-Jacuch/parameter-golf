...
# Execute before the rest of the script loads
HAS_FA3 = robust_fa3_setup()
print("=" * 60 + "\n")

# Now we can safely import or handle the fallback
try:
    if HAS_FA3:
...
class SwiGLURouter(nn.Module):
    """A high-capacity, low-rank dynamic router using SwiGLU logic."""
    def __init__(self, in_dim: int, out_dim: int, hidden_dim: int = 64):
        super().__init__()
        # Combine the up-projection and gate-projection into a single fast matmul
        self.w12 = nn.Linear(in_dim, hidden_dim * 2, bias=False)
        self.w3 = nn.Linear(hidden_dim, out_dim, bias=False)
        # ZERO-INIT MAGIC: Starts exactly as static KGIIR on step 1
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        hidden = F.silu(x1) * x2  # The SwiGLU activation
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.total_heads = num_heads + 2 * num_kv_heads
        self.total_dim = dim + 2 * self.kv_dim

        # --- 1. BASE KGIIR PARAMS ---
        self.q_decay = nn.Parameter(torch.full((dim,), -1.2))
        self.k_decay = nn.Parameter(torch.full((self.kv_dim,), -1.2))
        self.v_decay = nn.Parameter(torch.full((self.kv_dim,), -1.2))

        self.q_gate = nn.Parameter(torch.full((dim,), -1.5))
        self.k_gate = nn.Parameter(torch.full((self.kv_dim,), -1.5))
        self.v_gate = nn.Parameter(torch.full((self.kv_dim,), -1.5))

        self.q_imp = nn.Parameter(torch.tensor([-0.5, -1.5, -2.5]))
        self.k_imp = nn.Parameter(torch.tensor([-0.5, -1.5, -2.5]))
        self.v_imp = nn.Parameter(torch.tensor([-0.5, -1.5, -2.5]))

        # --- 2. SINGLE FUSED ROUTER ---
        # Predicts shifts for Q, K, and V in one massive parallel pass
        self.router = SwiGLURouter(in_dim=dim, out_dim=2 * self.total_heads, hidden_dim=64)
        ...
        if gated_attention:
            ...
            self.attn_gate._zero_init = True  # Protect zero-init from the global loop!
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor,
        ...
        # --- 1. FUSED DYNAMIC ROUTING ---
        dyn_shifts = self.router(x)  # [B, T, 2 * total_heads]
        decay_shift, gate_shift = dyn_shifts.reshape(bsz, seqlen, self.total_heads, 2).chunk(2, dim=-1)

        # --- 2. VECTORIZED MATH HACK (Denominator-Free) ---
        b_decay = torch.cat([self.q_decay, self.k_decay, self.v_decay], dim=-1).view(1, 1, self.total_heads, self.head_dim)
        b_gate = torch.cat([self.q_gate, self.k_gate, self.v_gate], dim=-1).view(1, 1, self.total_heads, self.head_dim)

        alpha = torch.sigmoid((b_decay + decay_shift).view(bsz, seqlen, self.total_dim)).to(dtype)
        g = torch.sigmoid((b_gate + gate_shift).view(bsz, seqlen, self.total_dim)).to(dtype)

        qi, ki, vi = torch.sigmoid(self.q_imp), torch.sigmoid(self.k_imp), torch.sigmoid(self.v_imp)
        i0 = torch.cat([qi[0].expand(dim), ki[0].expand(self.kv_dim), vi[0].expand(self.kv_dim)]).to(dtype)
        i1 = torch.cat([qi[1].expand(dim), ki[1].expand(self.kv_dim), vi[1].expand(self.kv_dim)]).to(dtype)
        ...
        # --- 3. SINGLE PASS KGIIR ---
        x_fused = torch.cat([x, x_kv, x_kv], dim=-1)  # [B, T, total_dim]
        x_pad = F.pad(x_fused, (0, 0, 3, 0))

        out = x_pad[:, 3:, :] * f0
        out.add_(x_pad[:, 2:-1, :] * f1)
        out.add_(x_pad[:, 1:-2, :] * f2)
        out.add_(x_pad[:, 0:-3, :] * f3)

        # Split back into Q, K, V
        q, k, v = torch.split(out, [dim, self.kv_dim, self.kv_dim], dim=-1)
        ...
        if v_embed is not None:
            v = v + v_embed.reshape_as(v).to(dtype)
        ...
        if self.gated_attention:
            # Gate operates on the original raw 'x' to maintain stable gradients
            y = y * torch.sigmoid(self.attn_gate(x)).unsqueeze(-1)
